...
export const useCartStore = create<CartStore>()(
  persist(
    (set, get) => ({
      ...
      subtotal: () => get().items.reduce((sum, item) => sum + (parseFloat(item.price.amount) * item.quantity), 0),
...
// Helper hook for cart sync on visibility change
export function useCartSync() {
  const syncCart = useCartStore(state => state.syncCart);
  
  if (typeof window !== 'undefined') {
    syncCart(); // Sync on initial load
